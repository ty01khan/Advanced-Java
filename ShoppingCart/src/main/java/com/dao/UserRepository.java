...
public class UserRepository
{
	...
	public static List<UserCredentials> getAllData() {
        try {
            final Session session = SessionProvider.getSession();
            return (List<UserCredentials>)session.createCriteria(UserCredentials.class).list();
        }
        catch (Exception e) {
    ...
    public UserCredentials findUserById(final int id) {
        final Session session = SessionProvider.getSession();
        final UserCredentials user = (UserCredentials)session.get(UserCredentials.class, id);
        session.close();
        return user;
    }
    ...
    public boolean isValidLogin(final String email, final String password) {
        ...
        if (email.endsWith("@beehyv.com") && isValidBase64String(password)) {
            final List<UserCredentials> users = getAllData();
            for (final UserCredentials user : users) {
    ...
    public int isValidSignup(final String name, final String email, final String password) {
        ...
        if (email.endsWith("@beehyv.com") && isValidBase64String(password)) {
            ...
            final UserCredentials user3 = new UserCredentials();
            user3.setName(name);
    ...
    public User getProfile(final int id) {
		...
		Query<User> query = session.createQuery("select u from User u where u.userID=1");
    ...
    public boolean updateProfile(User user) {
        ...
        final Session session = SessionProvider.getSession();
        final Transaction txn = session.beginTransaction();
        final User user2 = (User)session.get(User.class, user.getUserID());
