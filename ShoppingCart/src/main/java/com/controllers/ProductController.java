...
public class ProductController {
	...
	public ResponseEntity<Product> addProduct(@RequestBody final Product product) {
		...
		return new ResponseEntity<Product>(productRepo.addProduct(product), HttpStatus.OK);
	}

	@RequestMapping(value = { "/update" }, method = { RequestMethod.POST })
	public ResponseEntity<Product> modifyProduct(@RequestBody final Product product) {
		return new ResponseEntity<Product>(productRepo.updateProduct(product), HttpStatus.OK);
	}

	@RequestMapping(value = { "/getById/{productId}" }, method = { RequestMethod.GET }, produces = {
			"application/json" })
	public ResponseEntity<Product> getProductById(@PathVariable("productId") final int pid) {
		...
		return new ResponseEntity<Product>(productRepo.getProductById(pid), HttpStatus.OK);
	}
	...
	public ResponseEntity<List<Product>> getProductsByCategory(@PathVariable("category") final String category) {
		...
		return new ResponseEntity<List<Product>>(productRepo.getAllProductsByCategory(category), HttpStatus.OK);
	}

	@RequestMapping(value = { "/search/{searchString}" }, method = { RequestMethod.GET }, produces = {
			"application/json" })
	...
	public ResponseEntity<List<Product>> updateFilteredProduct(@RequestBody final Map<String, Integer> filter,
		...
		final List<Product> products = (List<Product>) this.productRepo.getFilteredProduct(category,
				(int) filter.get("minPrice"), (int) filter.get("maxPrice"));
